...
async (accessToken, refreshToken, profile, done) => {
  ...
  try {
    const [rows] = await connection.query('SELECT * FROM users WHERE facebookId = ?', [profile.id]);
    let user = rows[0];
...
passport.deserializeUser(async (id, done) => {
  try {
    const [rows] = await connection.query('SELECT * FROM users WHERE facebookId = ?', [id]);
    const user = rows[0];
    if (!user) {
